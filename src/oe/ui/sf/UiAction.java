...
//    @FXML
//    private static Label connectFailed1;
   private static Logger log = Logger.getLogger(UiAction.class.getName());
    ...
    public String getAssign_To__c(){
        if(action.getAssigned_To__c()==null){return null;}
        else {
        String searchQuery = "select id, name from user where id = '" + action.getAssigned_To__c()+"'";
        Conn conn = Conn.getInstance();
        SObject[] AssignToSObject = new SObject[0];
        try {
            AssignToSObject = conn.query(searchQuery);
        } catch (ConnectionException e) {
            log.error(e);
        }
        String AssignToName = ((User)AssignToSObject[0]).getName();
        return AssignToName;
        }
    }
